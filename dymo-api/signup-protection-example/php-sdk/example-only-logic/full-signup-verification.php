...
function checkSignUp(string $email, string $phone, string $ip, string $userAgent): array {
    ...
    try {
        ...
        if ($response->email->plugins->reachable === "invalid") return ["pass" => false, "message" => "Use an existing email."]; // Optional; the Reachable plugin already checks this internally.
